...
if user_input:
    with st.spinner("KRX API에서 코스피/코스닥 데이터 조회 중..."):
        try:
            # 코스피, 코스닥 데이터 가져오기
            kospi_df = fetch_krx_data(KOSPI_API_URL, base_date)
            kosdaq_df = fetch_krx_data(KOSDAQ_API_URL, base_date)
            ...
            if not kospi_match.empty:
                ...
                st.success("✅ 코스닥 종목 조회 성공")
                st.write(f"**시장:** 코스닥")
                st.write(f"**종목명:** {row['ISU_NM']}")
                st.write(f"**종목코드:** {row['ISU_SRT_CD']}")
                st.write(f"**상장주식수:** {int(row['LIST_SHRS'].replace(',', '')):,} 주")
            else:
                st.warning("❌ 입력한 종목을 찾을 수 없습니다.")
